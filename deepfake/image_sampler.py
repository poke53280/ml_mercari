...
orig_path = entry[0]
fake_path0 = entry[1][0]


orig_video = read_video(part_dir / orig_path, video_size)
fake_video = read_video(part_dir / fake_path0, video_size)
...
for x in range(4):
    ...
    #image_real = cv2.rectangle(image_real, (offset_lo[0], offset_lo[1]), (offste_hi[0], offste_hi[1]), (255,0,0), 5)

    #plt.imshow(image_real)
    #plt.show()


    real_sample = orig_video[int(video_size/2)][offset_lo[1]:offste_hi[1], offset_lo[0]:offste_hi[0]].copy()
    fake_sample = fake_video[int(video_size/2)][offset_lo[1]:offste_hi[1], offset_lo[0]:offste_hi[0]].copy()

    

    #image_diff = create_diff_image(real_sample, fake_sample)
    #plt.imshow(image_diff)
    #plt.show()


    image_3 = np.sum((real_sample-fake_sample)**2,axis=2)
    ...
    # Save mask: m
    # Save fake: fake_sample
    # Save real: real_sample

    # Fix


    real_sample = cv2.cvtColor(real_sample, cv2.COLOR_RGB2GRAY)
    fake_sample = cv2.cvtColor(fake_sample, cv2.COLOR_RGB2GRAY)
