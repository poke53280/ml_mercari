...
# BASED ON https://github.com/hollance/BlazeFace-PyTorch/. See LICENCE (Apache)
...
def blaze_on_frame(image, mtcnn_detector):
    ...
    for face in features_mtcnn:
        ...
        print(f"mtcnn. Box size {face['box'][2]}, {face['box'][3]}. confidence: {mtcnn_confidence}")

        x_size = x_max - x_min
        y_size = y_max - y_min

        c_x = int (.5 + x_min + x_size/2)
        c_y = int (.5 + y_min + y_size/2)


        half_size = 128//2

        c_x = int (np.min([np.max([c_x, half_size]), width - half_size]))
        c_y = int (np.min([np.max([c_y, half_size]), height - half_size]))

        im_min_x = c_x - half_size
        im_max_x = c_x + half_size

        im_min_y = c_y - half_size
        im_max_y = c_y + half_size

        assert im_max_x - im_min_x == half_size * 2
        assert im_max_y - im_min_y == half_size * 2

        assert im_min_x >= 0
        assert im_max_x <= width

        assert im_min_y >= 0
        assert im_max_y <= height

        img = image[im_min_x:im_max_x, im_min_y: im_max_y, :]
        ...
        anDetactions = np.array(detections)
        anDetactions = anDetactions * 128
        anDetactions = np.round(anDetactions)
        anDetactions = anDetactions.astype(np.int32)

        x_idx = [1, 3, 4, 6, 8, 10, 12, 14]
        y_idx = [0, 2, 5, 7, 9, 11, 13, 15]        

        anDetactions[:, x_idx] += im_min_x
        anDetactions[:, y_idx] += im_min_y

        for i in range(anDetactions.shape[0]):
            anFace = anDetactions[i]

            bb_min = (anFace[1], anFace[0])
            bb_max = (anFace[3], anFace[2])

            right_eye = (anFace[4], anFace[5])
            left_eye =  (anFace[6], anFace[7])

            nose =  (anFace[8], anFace[9])
            mouth = (anFace[10], anFace[11])

            right_ear = (anFace[12], anFace[13])        
            left_ear = (anFace[14], anFace[15])  

            confidence_score = anFace[16]
